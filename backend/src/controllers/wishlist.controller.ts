...
import type { CreateWishlistItemInput, UpdateWishlistItemInput } from '../validations/wishlist.validation.js';
...
export async function getWishlistItems(req: Request, res: Response): Promise<void> {
  ...
  // user_id is always the first condition — every query is scoped to the current user
  const filterParams: (string | number)[] = [req.user!.id];
...
export async function createWishlistItem(req: Request, res: Response): Promise<void> {
  const { title, price, priority, status } = req.body as CreateWishlistItemInput;

  const result = await pool.query(
    `INSERT INTO wishlist_items (title, price, priority, status, user_id)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING *`,
    [title, price, priority, status, req.user!.id],
  );
...
export async function updateWishlistItem(req: Request, res: Response): Promise<void> {
  const { id } = req.params;
  const { title, price, priority, status } = req.body as UpdateWishlistItemInput;

  const result = await pool.query(
    `UPDATE wishlist_items
     SET title = $1, price = $2, priority = $3, status = $4
     WHERE id = $5 AND user_id = $6
     RETURNING *`,
    [title, price, priority, status, id, req.user!.id],
  );
